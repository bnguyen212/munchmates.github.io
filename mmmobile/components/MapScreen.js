...
export default class MapScreen extends Component {
  ...
  setFilter(value) {
    ...
    switch (value) {
      ...
      case 1:
        color = 'blue';
        this.data = this.state.recommendations;
        break;
      case 2:
        color = 'yellow';
        this.data = this.state.favorites;
        break;
      case 3:
        color = 'green';
        this.data = this.state.upcoming
        break;
    }
    this.setState({filter: value, markerColor: color});
  }
        ...
        { !this.state.region ? <Text>Loading...</Text> : (
            ...
            <MapView
              ...
              { this.data.map((vendor, id) => (
                ...
                <Marker key={id} coordinate={vendor.coords} pinColor={this.state.markerColor}>
                  <Callout onPress={() => AlertIOS.alert('Favorited!')}>
                    <View style={{display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 300}}>
                      <Text style={{fontWeight: 'bold', fontSize: 15, marginBottom: 2, textAlign: 'center'}}>{vendor.name}</Text>
                      <View style={{display: 'flex', flexDirection: 'row', marginBottom: 2}}>
                        { vendor.desc.map((word, i) => (
                          <View key={i} style={{borderRadius: 20, padding: 5, marginHorizontal: 2, backgroundColor: 'black'}}>
                            <Text style={{color: iOSColors.yellow, fontSize: 10}}>{word}</Text>
                          </View>
                          ))
                        }
                      </View>
                      <Text style={{fontWeight: 'bold', fontSize: 15}}>{vendor.priceRange} | {vendor.type.toUpperCase()}</Text>
                    </View>
                  </Callout>
                </Marker>
                ))
            ...
            <View style={{position: 'absolute', bottom: '12%', left: '10%', right: '10%', zIndex: 2, opacity: 0.8}}>
